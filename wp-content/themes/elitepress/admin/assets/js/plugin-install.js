jQuery(document).ready(function($) {
    'use strict';
    var this_obj = elitepress_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'elitepress_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.webriti-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'elitepress_hide_customizer_notice';
            sdata.elitepress_plugin_slug = slug;
        }else{
            sdata.action = 'elitepress_hide_customizer_companion_notice';
        }
